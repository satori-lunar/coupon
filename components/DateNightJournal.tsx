...
interface DateNightJournalProps {
  dateNightId: string
  dateNightTitle: string
}

export default function DateNightJournal({ dateNightId, dateNightTitle }: DateNightJournalProps) {
  ...
  if (!isExpanded) {
    return (
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
      >
        {memory ? '📝 Document This Adventure' : '✨ Document This Adventure'}
      </motion.button>
    )
  }
    ...
    >
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-serif text-2xl md:text-3xl text-rose" style={{ fontFamily: 'var(--font-serif)' }}>
          Adventure Journal: {dateNightTitle}
        </h3>
        <button
      ...
      <div className="space-y-2">
        <label className="block text-warm-gray text-lg font-medium">How did it go?</label>
        <textarea
          value={notes}
          onChange={(e) => handleNotesChange(e.target.value)}
          placeholder="Write about your date night... what made it special? What did you learn about each other?"
